...
static unsigned int	get_pixel(t_img *img, unsigned x, unsigned y)
{
...
static void	ft_draw_ray(t_game *game, int x, float range, float angle)
{
	...
	h = (float)HEIGHT / (range * cosf(angle));
	y = 0;
	while (y < HEIGHT)
	{
...
static void	ft_cast_rays(t_game *game)
{
	...
	ray = *game->plr;
	ray.start = ray.angle - game->fov / 2.00;
	ray.end = ray.angle + game->fov / 2.00;
	while (d_x < WIDTH)
	{
...
void put_men_map(t_game *game)
{
	...
	while (i < (HEIGHT / 70) / 4)
	{
		...
		while (j < (HEIGHT / 70) / 4)
		{
			put_pixel(game->draw, (game->plr->x) * (HEIGHT / 70) + j, (game->plr->y) * (HEIGHT / 70) + i, 0x000000FF);
			j++;
		}
		i++;
	}	
...
void draw_minimap(t_game *game)
{
	...
	while (y < game->h)
	{
		...
		{
			if (game->map[y][x] == '1')
				put_rectanlge_map(game, x * (HEIGHT / 70), y * (HEIGHT / 70), 100);
			x++;
		}
		y++;
	}
